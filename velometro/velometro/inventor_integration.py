...
def update_description(doc, method):
	...
	if doc.get("manufacturer") is not None And doc.get("manufacturer") != "":
		mfg = doc.get("manufacturer")
		mfg = mfg.encode('utf-8')
	if doc.get("manufacturer_part_no") is not None And doc.get("manufacturer_part_no") != "":
		try:
			...
			try:
				description = notes + " (".encode('utf-8') + mfg.encode('utf-8') + " PN: ".encode('utf-8') + doc.get("manufacturer_part_no").encode('utf-8') + ")".encode('utf-8')
			except:
				description = notes
	else:
		description = notes
